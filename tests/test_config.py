...
def test_load_config_cloud_success() -> None:
    with patch.dict(os.environ, {"JWRAG_ENGINE": "cloud", "JWRAG_CLOUD_API_KEY": "sk-test"}, clear=True):
        config = load_config()
...
def test_load_config_cloud_missing_key() -> None:
    with patch.dict(os.environ, {"JWRAG_ENGINE": "cloud"}, clear=True):
        with pytest.raises(ValueError, match="JWRAG_CLOUD_API_KEY must be set"):
            load_config()

def test_load_config_unknown_engine() -> None:
    with patch.dict(os.environ, {"JWRAG_ENGINE": "magic"}, clear=True):
