...
def test_synthesize_success(engine: OllamaSynthesisEngine) -> None:
    mock_response = MagicMock()
    mock_response.json.return_value = {"response": '{"options": [{"title": "A", "reasoning": "R", "conclusions": ["C1"]}]'}
    mock_response.raise_for_status = MagicMock()
    
    with patch.object(engine.client, 'post', return_value=mock_response):
        chunks = [Chunk(id="1", document_id="d1", chunk_index=0, text_content="Context A", embedding=np.zeros(3), metadata={})]
...
def test_synthesize_json_parsing_fallbacks(engine: OllamaSynthesisEngine) -> None:
    ...
    mock_response2.json.return_value = {"response": "Some text before {\"options\": [{\"title\": \"C\", \"reasoning\": \"R\", \"conclusions\": []}]} after text"}
